...
var {deleteUserAccount} = require("../controllers/users");
...
router.get("/settings", function(req, res, next) {
    db.findById(req.session._id, async (err, user) => {
      let userGroups = user.groups.map(async (groupId) => {
            return await groupHandler.asyncFindById(groupId);
      });
...
router.get("/deleteUserAccount", function(req, res, next) {
  db.findOne({ _id: req.session._id }, (err, user) => {
...
router.delete("/userAccount/:userId", async function(req, res, next) {
  const userIdToDelete = req.params.userId;
    
  deleteUserAccount({_id: userIdToDelete});
        ...
        function(error, result) {
          ...
          if (!error) {
            ...
            if (req.body.type == "My Feed"){
              ...
              groupHandler.findOne({name: req.body.type}, (err, group) => {
                  if(group!=undefined) {
                group.posts.push({
                  _id:random_id,
                  author:req.session.user,
